...
// bd44075 (removido quando a nav era um carrossel), só que agora fica
// atrás do ícone de CADA item (um por slot da grade fixa), aparecendo via
// opacity/scale quando o item ganha .active, em vez de remontar/viajar até
// o centro. useId garante gradientes com id único por instância — sem
// isso, as 5 bolas montadas ao mesmo tempo colidiriam no mesmo <defs>.
export default function BottomNavBall() {
  const id = useId();
  const corpoId = `pnb-corpo-${id}`;
  const anelId = `pnb-anel-${id}`;
  return (
    <svg className="pl-nav-ball-svg" viewBox="0 0 32 32" aria-hidden="true">
      <defs>
