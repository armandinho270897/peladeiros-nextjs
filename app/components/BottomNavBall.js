...
//
// Dois elementos, não um: o wrapper leva a inclinação por item (--tilt,
// estática, definida por posição no CSS) e a svg leva a animação de
// entrada (escala+fade+rotação). Um <keyframes> que lê var(--tilt) direto
// reinicia sozinho toda vez que qualquer recálculo de estilo na página
// toca essa variável (bug real de engine, confirmado ao vivo — a animação
// nunca chegava a terminar, ficava presa em currentTime:0 pra sempre).
// Separar em dois elementos tira a variável de dentro do @keyframes.
export default function BottomNavBall() {
  return (
    <span className="pl-bottom-nav-ball-wrap" aria-hidden="true">
      <svg className="pl-bottom-nav-ball" viewBox="0 0 24 24" fill="none">
