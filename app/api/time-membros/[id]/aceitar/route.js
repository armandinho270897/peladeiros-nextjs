...
export async function POST(request, { params }) {
  if (!checkRateLimit(`time-membros:aceitar:${getClientIp(request)}`)) {
  ...
  for (const c of capitaes || []) {
    await createNotification({
      userId: c.user_id,
      tipo: 'convite_time_aceito',
      mensagem: `${profile?.nome || 'Um jogador'} aceitou seu convite pro time ${time?.nome || ''}.`,
      atorUserId: user.id,
    });
  }
