import BaseballGloveIcon from '@/app/components/icons/BaseballGloveIcon';
import TicketIcon from '@/app/components/icons/TicketIcon';
import CheckMarkIcon from '@/app/components/icons/CheckMarkIcon';
import HazardSignIcon from '@/app/components/icons/HazardSignIcon';
import StopwatchIcon from '@/app/components/icons/StopwatchIcon';
import CrossMarkIcon from '@/app/components/icons/CrossMarkIcon';
import HourglassIcon from '@/app/components/icons/HourglassIcon';
import PositionMarkerIcon from '@/app/components/icons/PositionMarkerIcon';
import TalkIcon from '@/app/components/icons/TalkIcon';
import TShirtIcon from '@/app/components/icons/TShirtIcon';
import TrophyIcon from '@/app/components/icons/TrophyIcon';
import ExitDoorIcon from '@/app/components/icons/ExitDoorIcon';
import CrownIcon from '@/app/components/icons/CrownIcon';
...
export const NOTIF_TIPOS = [
  ...
  // partida_proxima: tipo antigo, único (cron de 24h e checagem reativa de
  // 3h disputavam o mesmo dedup — quem chegasse primeiro "consumia" o
  // aviso e a outra janela nunca disparava). Mantido aqui só pra linhas
  // antigas no banco continuarem renderizando certo; escritas novas usam
  // os dois tipos abaixo, com dedup independente (migration 036).
  { id: 'partida_proxima', categoria: 'urgente', Icone: StopwatchIcon, cor: 'neon', label: 'Sua partida está próxima' },
  // Rótulo genérico de propósito — o cron roda 1x/dia (varredura de até
  // 24h à frente), então pode pegar uma pelada faltando só algumas horas,
  // não exatamente 24h. A mensagem de cada aviso (lib/lembretesPartida.js)
  // sempre usa a contagem REAL de horas restantes; esse label só precisa
  // não prometer uma precisão que a varredura não tem.
  { id: 'partida_proxima_24h', categoria: 'urgente', Icone: StopwatchIcon, cor: 'neon', label: 'Partida próxima' },
  { id: 'partida_proxima_3h', categoria: 'urgente', Icone: StopwatchIcon, cor: 'neon', label: 'Sua partida é em breve' },
  { id: 'checkin_lembrete', categoria: 'urgente', Icone: CheckMarkIcon, cor: 'neon', label: 'Lembrete de check-in' },
  ...
  // Moderação de conta (área de Administração) — advertência/suspensão/
  // bloqueio são urgentes (afetam o que dá pra fazer no app agora, e
  // mandam e-mail); reativação é boa notícia, sem pressa nenhuma.
  { id: 'conta_advertida', categoria: 'urgente', Icone: HazardSignIcon, cor: 'concrete', label: 'Sua conta recebeu uma advertência' },
  { id: 'conta_suspensa', categoria: 'urgente', Icone: HazardSignIcon, cor: 'concrete', label: 'Sua conta foi suspensa' },
  { id: 'conta_bloqueada', categoria: 'urgente', Icone: TrashCanIcon, cor: 'concrete', label: 'Sua conta foi bloqueada' },
  { id: 'conta_reativada', categoria: 'comunidade', Icone: CheckMarkIcon, cor: 'neon', label: 'Sua conta foi reativada' },
  { id: 'denuncia_resolvida', categoria: 'comunidade', Icone: CheckMarkIcon, cor: 'neon', label: 'Uma denúncia sua foi analisada' },
  { id: 'pelada_pausada_admin', categoria: 'urgente', Icone: EditIcon, cor: 'concrete', label: 'Uma pelada sua foi pausada pela administração' },
  { id: 'pelada_cancelada_admin', categoria: 'urgente', Icone: TrashCanIcon, cor: 'concrete', label: 'Uma pelada sua foi cancelada pela administração' },
  { id: 'encerrar_partida_pendente', categoria: 'comunidade', Icone: HourglassIcon, cor: 'lilas', label: 'Uma pelada sua está esperando ser encerrada' },
];
