...
// Anexa nota_media, moral, modalidade/posicoes e peladas_jogadas em cada
// confirmacao, com um punhado de queries agregadas em memória — evita N+1
// mesmo com várias peladas/confirmados na tela. Usado tanto no GET de
// games quanto no de games/[id], e o resumo (posições/nota/histórico)
// alimenta a lista de solicitações pendentes na tela de gerenciar.
export async function attachNotaMedia(games) {
  ...
  const [{ data: avaliacoes }, { data: perfis }, { data: historico }, { data: cancelamentos }] = await Promise.all([
    ...
    supabase.from('profiles').select('id, modalidade_principal, posicoes, created_at').in('id', idsArr),
