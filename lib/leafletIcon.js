...
// Área de toque maior que o círculo visível (52px em vez de 38px) — achado
// ao vivo: um toque levemente impreciso no pin (comum em tela pequena)
// registrava como arrastar o mapa em vez de tocar na arena, caindo no
// reverse-geocode genérico em vez de abrir o sheet da arena. O wrapper
// .pl-map-arena-token-hit é o elemento de verdade do ícone do Leaflet (é
// ele que recebe o clique) — o círculo de 38px continua do mesmo tamanho
// visualmente, só centralizado dentro de uma área clicável maior.
export function arenaTokenPinIcon(arena, temJogo) {
  ...
  return L.divIcon({
    className: '',
    html: `<div class="pl-map-arena-token-hit">
      <div class="pl-map-arena-token-pin ${classe}">
        ${!pendente ? '<span class="pl-map-arena-token-glow"></span>' : ''}
        <div class="pl-map-arena-token-photo">${miolo}</div>
        ${pendente ? `<span class="pl-map-arena-token-badge-clock" title="Aguardando aprovação"><svg width="9" height="9" viewBox="0 0 24 24" fill="none"><circle cx="12" cy="12" r="9" stroke="#0A0A0A" stroke-width="2.2"/><path d="M12 7v5l3.2 2" stroke="#0A0A0A" stroke-width="2.2" stroke-linecap="round"/></svg></span>` : ''}
