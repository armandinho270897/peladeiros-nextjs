...
// BASE = distância até o primeiro vizinho; PASSO = incremento por posição
// adicional. Curva elíptica sutil via curvaY (nada de perspectiva 3D
// pesada) e uma leve inclinação (tiltDeg) por passo pra dar sensação de
// roda sem exagerar.
const BASE_X = 52;
const PASSO_X = 46;
const CURVA_Y = 3.4;
...
const ESCALA = { 0: 1, 1: 0.8, 2: 0.56 };
const OPACIDADE = { 0: 1, 1: 0.82, 2: 0.46 };
...
// offset -> transform/opacity do slot. |offset| pode passar de OFFSET_MAX
...
export function geometriaDoOffset(offset) {
  ...
  return {
    ...
    tilt: sinal * TILT_DEG * Math.min(abs, OFFSET_MAX) / OFFSET_MAX,
