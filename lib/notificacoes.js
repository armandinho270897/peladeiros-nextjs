...
// Nome do autor em negrito onde aparecer dentro da mensagem — o texto já
// vem pronto do backend (ex: "${profile.nome} pediu pra entrar..."), então
// só localizamos a mesma string em vez de reconstruir a frase. Usado tanto
// por /avisos quanto pelo feed resumido da Home.
export function comNomeEmNegrito(mensagem, nome) {
  if (!nome) return mensagem;
  const partes = mensagem.split(nome);
  if (partes.length === 1) return mensagem;
  return partes.flatMap((parte, i) => (i === 0 ? [parte] : [<b key={i}>{nome}</b>, parte]));
}
