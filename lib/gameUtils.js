...
// "Hoje" sempre no horário de Brasília — o servidor roda em UTC, onde já é
// "amanhã" a partir das 21h daqui, e isso sumia com peladas da noite e
// virava o mês da mensalidade três horas cedo. `agora` é injetável só pra
// teste (tests/horario-brasilia.test.mjs) não depender de mockar Date
// globalmente, API ainda instável entre versões do Node.
export function todayISO(agora = new Date()) {
  return new Intl.DateTimeFormat('en-CA', { timeZone: 'America/Sao_Paulo' }).format(agora);
}

// Primeiro dia do mês corrente (Brasília), no formato de mensalidades.mes_referencia.
export function mesAtualISO(agora = new Date()) {
  return todayISO(agora).slice(0, 8) + '01';
}

// Data ISO deslocada N dias — mesma lógica que já existia inline em
// app/peladas/page.js (amanhaISO), generalizada aqui pra também servir o
// filtro "fim de semana" da Descoberta (lib/gameUtils.js#fimDeSemanaRange)
// sem duplicar a aritmética de data em dois lugares.
export function addDiasISO(hojeISO, dias) {
  const [y, m, d] = hojeISO.split('-').map(Number);
  const dt = new Date(y, m - 1, d + dias);
  return dt.getFullYear() + '-' + String(dt.getMonth() + 1).padStart(2, '0') + '-' + String(dt.getDate()).padStart(2, '0');
}

// Próximo sábado e domingo a partir de hoje (inclusive, se hoje já for
// sábado ou domingo) — usado pelo filtro de data "Fim de semana" da
// Descoberta. getDay(): 0=domingo...6=sábado.
export function fimDeSemanaRange(hojeISO) {
  const [y, m, d] = hojeISO.split('-').map(Number);
  const hoje = new Date(y, m - 1, d);
  const diaSemana = hoje.getDay();
  const diasAteSabado = diaSemana === 6 ? 0 : diaSemana === 0 ? 6 : 6 - diaSemana;
  const inicio = addDiasISO(hojeISO, diasAteSabado);
  const fim = addDiasISO(inicio, diaSemana === 0 ? 0 : 1);
  return { inicio, fim };
}

// Período do dia a partir do horário (string "HH:MM" ou "HH:MM:SS") — usado
// pelo filtro "Manhã/Tarde/Noite" da Descoberta. Faixas simples, sem
// pretensão de exatidão astronômica: manhã até 12h, tarde até 18h, noite
// depois disso.
export function periodoDe(horario) {
...
export const PERIODO_LABEL = { manha: 'Manhã', tarde: 'Tarde', noite: 'Noite' };

// Mesmas 4 opções já digitadas inline em NewGameModal.js/ManageModal.js
// (nível da PELADA — não confundir com times.nivel_competitivo, conceito
// separado em lib/timeConstants.js). Só os filtros da Descoberta usam essa
// constante por ora; os dois formulários continuam com a lista inline pra
// não mexer em código já funcionando sem necessidade.
export const NIVEL_OPCOES = ['Iniciante', 'Intermediário', 'Avançado', 'Qualquer nível'];
...
// O capitão é sempre presença confirmada na própria pelada, independente de
// existir ou não a linha dele em confirmacoes (ele nunca "pede" pra jogar o
// próprio jogo) — usado tanto pro status exibido em GameCard quanto pra
// autorização do chat (lib/chatAuth.js).
export function souCapitaoDe(game, userId) {
  return !!game?.owner_id && game.owner_id === userId;
}
...
// condição, um lugar só. Capitão conta como aprovado mesmo sem linha em
// confirmacoes (defesa em profundidade — ver souCapitaoDe).
export function souAprovadoDe(g, userId) {
  return souCapitaoDe(g, userId) || (g.confirmacoes || []).some((c) => c.user_id === userId && c.status === 'aprovado');
}
...
// Selo de vitória/empate/derrota no histórico do Perfil — só existe quando
// a pelada teve times A/B montados (MontarTimesModal) e placar registrado
// (calculado em app/api/perfil/route.js). Cor: neon = ganhou, vermelho =
// perdeu, cinza neutro = empatou.
export const RESULTADO_LABEL = { vitoria: 'Vitória', empate: 'Empate', derrota: 'Derrota' };
export const RESULTADO_COR = { vitoria: 'var(--neon)', empate: 'var(--paper-dim)', derrota: 'var(--tag-red)' };
...
// "Começa em breve": usado pro glow no pin do mapa (MapViewPins) — jogo
// ainda não rolou, mas falta uma hora ou menos pro início. Calculado uma
// vez no momento em que a lista de jogos chega (não fica reavaliando a
// cada segundo) — o mapa não tem um "agora" vivo tickando como o card tem.
export const COMECA_EM_BREVE_MS = 60 * 60 * 1000;

export function comecaEmBreve(game) {
  if (!game?.data || !game?.horario) return false;
  const diff = inicioDoJogo(game).getTime() - Date.now();
  return diff >= 0 && diff < COMECA_EM_BREVE_MS;
}
...
// Check-in: janela abre 60min antes do início e fica aberta até o
// organizador encerrar a partida (sem teto de horário — combinar com
// jaAconteceu seria bloquear check-in bem na hora que ele mais serve).
// Tolerância de 10min pra pontualidade — depois disso, atraso. Desfazer só
// nos primeiros 5min após o check-in (evita ficar reversível pra sempre).
export const CHECKIN_JANELA_ANTES_MS = 60 * 60 * 1000;
export const CHECKIN_TOLERANCIA_MS = 10 * 60 * 1000;
export const CHECKIN_UNDO_MS = 5 * 60 * 1000;

export function checkinAbreEm(game) {
  return new Date(inicioDoJogo(game).getTime() - CHECKIN_JANELA_ANTES_MS);
}

export function checkinJanelaAberta(game) {
  if (!game?.data || !game?.horario || game.encerrada_em) return false;
  return Date.now() >= checkinAbreEm(game).getTime();
}

export function checkinPontual(checkinAtISO, game) {
  if (!checkinAtISO) return null;
  return new Date(checkinAtISO).getTime() <= inicioDoJogo(game).getTime() + CHECKIN_TOLERANCIA_MS;
}

// Status de check-in de UMA confirmação pra exibição (painel do
// organizador) — fonte única, pra não reimplementar essa lógica em cada
// tela. Só faz sentido pra quem está 'aprovado' (só esse status pode fazer
// check-in); quem foi marcado presente manualmente (sem checkin_at) não
// tem horário confiável pra virar "atrasado" — vira 'presente_manual'.
// HH:MM sempre no horário de Brasília, independente do fuso do navegador
// de quem tá vendo — mesma garantia que inicioDoJogo já dá pro cálculo,
// agora pra exibição ("check-in abre às HH:MM").
export function formatHoraSP(date) {
  return new Intl.DateTimeFormat('pt-BR', { timeZone: 'America/Sao_Paulo', hour: '2-digit', minute: '2-digit' }).format(date);
}

export function statusCheckin(c, game) {
  if (!c || c.status !== 'aprovado') return null;
  if (c.presente === false) return 'falta';
  if (c.checkin_at) return checkinPontual(c.checkin_at, game) ? 'pontual' : 'atrasado';
  if (c.presente === true) return 'presente_manual';
  return 'sem_checkin';
}
